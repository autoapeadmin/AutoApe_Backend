 ...
 * @category        Libraries
 * @author          Phil Sturgeon, Chris Kacerguis, Edward An
 * @license         MIT
 * @version         1.0.2
 */
abstract class REST_Controller extends CI_Controller {
    ...
    /**
     * @access rochas
     * Note: Common variable for PropertiMax
     * 
     * @category Version History
     * @version 1.0.2   Edward An - Init
     *          1.0.1   Edward An - Development of a RESTful API Framework
     *          1.0.2   Edward An - Manager API Development
     */
    const REST_VERSION = '1.0.2';
    const LOYALTY_PERCENT = 5;
    const ROYALTY_TO_DALAR = 100;


    // // for Customer
    // const SESS_CUSTOMER_ID    = "CUSTOMER_ID";
    // const SESS_SIGN_FLAG      = "SIGN_FLAG";
    // const SESS_TOKEN_CUSTOMER = "TOKEN_CUSTOMER";
    // const SESS_TOKEN_CART     = "TOKEN_CART";
    // const SESS_TOKEN_CART_ID  = "TOKEN_CART_ID";
    
    // // for Manager
    // const SESS_MANAGER_ID     = "MANAGER_ID";
    // const SESS_TOKEN_MANAGER  = "TOKEN_MANAGER";
    // const SESS_ADMIN_FLAG     = "ADMIN_FLAG";

    // const SESS_SITE_ASSIGN    = "SITE_ASSIGN";

    // const SESS_FIRST_NAME     = "SESS_FIRST_NAME";
    // const SESS_LAST_NAME      = "SESS_LAST_NAME";
     ...
     * Parameters: input_passwd, db_passwd
     */
    protected function savelog($agent_id, $function_name) { // 22-Feb-18 - Checked
        // $data = array();
        // $data['agent_id'] = $agent_id;
        // $data['function_name'] = $function_name;

        // $result = $this->REST_Model->insertLog($data);
        $result = TRUE;
    }
    ...
    protected function uniqid_pic() {
        $uidpic = md5(uniqid(rand(), true));
        return $uidpic;
    }
    ...
    protected function getDate() {
        $get_date = date_create('now')->format('Y-m-d');
        return $get_date;
    }

    protected function json_encode_msgs($val, $msg = "success") {
        $msgx['errorcode'] = 0;
        $msgx['errormsg'] = $msg;
        $msgx['data'] = $val;
        return $this->output
    ...
    protected function saveImageToPath($path, $pic_name, $files, $file_name, $file_size) {
        ...
        if ($file_size > 0) {
            for ($i = 0; $i < $file_size; $i++) {
                if ($files[$file_name]['name'][$i]) {
                    $tempFileInfo = array();
                    $config['file_name'] = $pic_name.$i;
                    try {
                        ...
                        $picNames[$i]['name'] = $pic_name.$i.".".explode(".", strtolower($files[$file_name]['name'][$i]))[1];
                    } catch (Exception $e) {
                        $picNames[$i]['name'] = "";
                        continue;
                    }
    ...
    protected function sendResetPasswordEmail($email, $name, $temp_password) {
        ...
        $this->email->from(self::MAIN_EMAIL_SENDER, 'SmartCart');
        $this->email->to($email);
                                    ...
                                    <tr style="'.$style_sets.' margin: 0;">
                                        <td class="content-wrap" style="'.$style_sets.' vertical-align: top; margin: 0; padding: 20px;" valign="top">
                                            <table width="100%" cellpadding="0" cellspacing="0" style="'.$style_sets.' margin: 0;">
                                                ...
                                                <tr style="'.$style_sets.' margin: 0;">
                                                    <td class="content-block" style="'.$style_sets.' vertical-align: top; margin: 0; padding: 0 0 20px;" valign="top">If you want to change to the password you want, first need to log in with this temporary password and edit it with the profile.</td>
                                                </tr>
                                                <tr style="'.$style_sets.' margin: 0;">
                                                    <td class="content-block" style="'.$style_sets.' vertical-align: top; margin: 0; padding: 0 0 20px;" valign="top">With Smart Cart.</td>
                                                </tr>
                                            </table>
                                        </td>
                                    </tr>
    ...
    // Interlocks with the Foodstuff API
    protected function getFromUrl($url, $method = 'GET') {
        $ch = curl_init();
        $curlopt_id = 'ck_7fe3ba80aa3d5f32827d4960625fe6c1c813041e';
        $curlopt_pw = 'cs_e41f2300dfa7912ba369747bb0a5120bab520aa2';
        ...
        switch(strtoupper($method))
        {
            case 'GET':
                ...
                curl_setopt($ch , CURLOPT_USERPWD, $curlopt_id.':'.$curlopt_pw);
                break;
     ...
     */
    protected function getFoodstuffInfo($product_sku) {
        $now_date = $this->getDate();
        $cache_data = $this->REST_Model->selectCache($product_sku, $now_date);

        if ($cache_data->num_rows() > 0) {
            $result = $cache_data->result();
            return $result[0];

        } else {
            $base_url = 'https://ellerslie.sotftest.4square.kiwi';
            
            $json_result = $this->getFromUrl($base_url.'/wc-api/v3/pos_aux_product/'.$product_sku, 'GET');
            $array_result = json_decode($json_result, false);
            $product_id = $array_result->id;

            if ($product_id){
                $product_init = $this->getFromUrl($base_url.'/wc-api/v3/products/'.$product_id, 'GET');
                $product_info = json_decode($product_init, false);
                $product_data = $product_info->product;

                if (!empty($product_data->id)) {
                    $this->setFoodstuffInfo($product_data);
                }
    ...
    /**
     * Requirements:
     * Parameters: $token_cart
     */
    protected function getCacheShoppingInfo($token_cart) {
        $shopping_list = $this->REST_Model->selectShoppingCart($token_cart);
        $cnt_array = count($shopping_list);
        $product_list = array();
        
        if ($cnt_array > 0) {
            $total_row = 0; // Check cart_quantity count
            $total_sum = 0; // Check cart_quantity count

            $cart_info = array();
            $cart_info['cart_id'] = $shopping_list[0]->cart_id;
            $cart_info['cart_code'] = $shopping_list[0]->cart_code;
            $cart_info['cart_name'] = $shopping_list[0]->cart_name;
            $cart_info['token_cart'] = $shopping_list[0]->token_cart;
            $cart_info['cart_status'] = $shopping_list[0]->cart_status;
            $cart_info['cart_location'] = $shopping_list[0]->cart_location;
            $cart_info['cart_pair'] = $shopping_list[0]->cart_pair;
            $cart_info['cart_paid'] = $shopping_list[0]->cart_paid;

            for ($irow = 0; $irow < $cnt_array; $irow++) {
                $get_shopping_list = $shopping_list[$irow];

                if ($get_shopping_list->cart_quantity > 0) {
                    $get_sku_id = $get_shopping_list->barcode_id;
                    $showview = $this->getFoodstuffInfo($get_sku_id);
    
                    $shopping_data = array();
                    $shopping_data['barcode_id'] = $get_shopping_list->barcode_id;
                    $shopping_data['cart_quantity'] = $get_shopping_list->cart_quantity;
                    $shopping_data['image_id'] = $get_shopping_list->image_id;
                    $shopping_data['title'] = $showview->title;
                    $shopping_data['id'] = $showview->id;
                    // $shopping_data['created_at'] = $showview->created_at;
                    // $shopping_data['updated_at'] = $showview->updated_at;
                    $shopping_data['type'] = $showview->type;
                    $shopping_data['status'] = $showview->status;
                    $shopping_data['categories'] = $showview->categories;
                    $shopping_data['weight'] = $showview->weight;
                    $shopping_data['description'] = $showview->description;
                    // $shopping_data['downloadable'] = $showview->downloadable;
                    // $shopping_data['virtual'] = $showview->virtual;
                    // $shopping_data['permalink'] = $showview->permalink;
                    $shopping_data['sku'] = $showview->sku;
                    $shopping_data['price'] = $showview->price;
                    $shopping_data['brand'] = $showview->brand;
                    $shopping_data['regular_price'] = $showview->regular_price;
                    $shopping_data['sale_price'] = $showview->sale_price;
                    $temp_sum = (float)$showview->price * (int)$get_shopping_list->cart_quantity;
                    $shopping_data['sum_price'] = $temp_sum;
                    $product_list[$total_row] = $shopping_data;
        ...
        $result = array();
        $result["total_items"] = $cnt_array;
        $result["total_price"] = round($total_sum, 2);

        $nz_gst = self::NZ_GST;
        if ($nz_gst > 99) {
            $result["total_gst"] = round($total_sum - (($total_sum / $nz_gst) * 100) ,2);
        } else {
            $result["total_gst"] = round((($total_sum * $nz_gst) / 100) ,2);
        }

        $result["cart_info"] = $cart_info;
        if ($cnt_array > 0) {
            $result["shopping_cart"] = $product_list;
        }
        $result["shopping_cart_size"] = sizeof($product_list);
     ...
     * Parameters: $product_sku
     */
    protected function getPaymentInfo($product_sku, $now_date) {
        $cache_data = $this->REST_Model->selectCache($product_sku, $now_date);
        $result = $cache_data->result();
        return $result[0];
    }

    protected function setFoodstuffInfo($product_data) {
        $data = array();
        $data['cache_idate'] = $this->getDate();
        $data['title'] = $product_data->title;
        $data['id'] = $product_data->id;
        $data['type'] = $product_data->type;
        $data['status'] = $product_data->status;
        $data['categories'] = $product_data->categories[0];
        ...
        $data['regular_price'] = $product_data->regular_price;
        $data['sale_price'] = $product_data->sale_price;
        $result = $this->REST_Model->insertCache($data);
        return $result;
    }
     ...
     * @author Edward An
     */
    public function res_true($results, $message)
    {
        // Get the current timestamp
        $result = array();
        $result['version'] = self::REST_VERSION;
        $result['status'] = true;
    ...
    public function res_false($results, $message)
    {
        // Get the current timestamp
        $result = array();
        $result['version'] = self::REST_VERSION;
    ...
    public function res_format($results, $message)
    {
        // Get the current timestamp
        $result = array();
        $result['version'] = self::REST_VERSION;
