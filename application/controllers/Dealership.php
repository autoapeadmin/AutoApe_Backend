 ...
 * @license         MIT
 */
class Dealership extends Base_controller
{
    ...
    {
        $this->load->view('dealership/dashboard.php');
    }

    public function logout()
    {
        session_unset();
        redirect(base_url("/dealership/login"), 'refresh');
    }
    ...
    public function dashboard()
    {
        if (!isset($_SESSION['DEALERSHIP'])) {
            ...
            $id = $_SESSION["DEALERSHIP"];
            $data2["nCustomer"] = $this->Dealership_Model->selectDashSalesConsultants($id);

            $_SESSION["newMessages"] = $this->Dealership_Model->selectDashNewMessages($id);

            //mensajes nuevos

            $data2["total_month"] = $this->countTotalMonthly($id);
            $data2["subscriptiondata"] = $this->Dealership_Model->selectSubscriptionByDealership($id);

            $_SESSION["lastTwoMessages"] = $this->Dealership_Model->findMessageByDealership2($id);

            $dat2 = $this->Dealership_Model->selectDashLastSold($id);
            $dat3 = $this->Dealership_Model->selectSoldThisMonth($id);
            ...
            foreach ($dat2 as $key) {
                ...
                if (isset($dat[0]->pic_url)) {
                    $key->pic_url = "https://d2yab6hrhf5a6v.cloudfront.net/200x200/maxauto/listingCar/" .  $dat[0]->pic_url;
                }
                array_push($veh, $key);
            }

            $data2["lastSold"] = $veh;
            $data3["graph"] = $this->getLastMonthSells();
            $data3["graph2"] = $this->getLastMonthPost();
            $data2["soldmo"] = $dat3;

            //$data2["nSubs"] = $this->Dealership_Model->selectDashSubsc();
            //$data2["nDealer"] = $this->Dealership_Model->selectDashDealership();
            $this->load->view('dealership/includes/header.php', $data2);
            $this->load->view('dealership/dashboard.php');
            $this->load->view('dealership/includes/footer.php', $data3);
        }
    }

    public function getLastMonthSells()
    {
        $id = $_SESSION["DEALERSHIP"];
        $valuesDB = $this->Dealership_Model->selectDashboardGraph($id);

        // the last 6 month
        for ($i = 0; $i <= 4; $i++) {

            $month =  date('n', strtotime('last day of ' . -$i . 'month'));
            $year =  date('Y', strtotime('last day of ' . -$i . 'month'));
            $monthName = date("F", mktime(0, 0, 0, $month, 10));

            $into = false;
            $valuesMonthYear = [];
            for ($x = 0; $x < sizeOf($valuesDB); $x++) {

                if ($valuesDB[$x]->month == $month && $valuesDB[$x]->year == $year) {
                    $valuesMonthYear["date"] =  $monthName . " " . $year;
                    $valuesMonthYear["total"] = $valuesDB[$x]->total;
                    $into = true;
                }
            }
            ...
            if (!$into) {
                $valuesMonthYear["date"] = $monthName . " " . $year;
                $valuesMonthYear["total"] = "0";
            }
            $values[$i] = $valuesMonthYear;
        }

        $retur =  array_reverse($values);
    ...
    {
        $id = $_SESSION["DEALERSHIP"];
        $valuesDB = $this->Dealership_Model->selectDashboardGraphListed($id);
    ...
    {
        $data2["page"] = "createvehicle";

        $data["makes"] = $this->Maxauto_Model->selectAllMake();
        $data["makes2"] = $this->Maxauto_Model->selectAllMakeMotoSearch();
        $data["models"] = $this->Maxauto_Model->selectAllMake();
        $data["fuelType"] = $this->Maxauto_Model->selectAllMake();

        $data['bodyTypeCar'] = $this->Maxauto_Model->selectABodiesById("0");
        $data['bodyTypeMoto'] = $this->Maxauto_Model->selectABodiesById("1");


        $this->load->view('dealership/includes/header.php', $data2);
        $this->load->view('dealership/listvehicle.php', $data);
        $this->load->view('dealership/includes/footer.php');
    }

    public function edit_vehicle($id)
    {
        $data2["page"] = "editvehicle";
        ...
        $data["fuelType"] = $this->Maxauto_Model->selectAllMake();
        $data['bodyTypeCar'] = $this->Maxauto_Model->selectABodiesById("0");
        $data['bodyTypeMoto'] = $this->Maxauto_Model->selectAllBodies("1");

        $veh2 = $this->Dealership_Model->findVehiculeById($id);
        ...
        foreach ($veh2 as $key) {
            //get image
            $val =  $key->vehicule_id;
            $photos = $this->Dealership_Model->getAllImages($val);
        }
        ...
        $this->load->view('dealership/includes/header.php', $data2);
        $this->load->view('dealership/editvehicle2.php', $data);
        $this->load->view('dealership/includes/footer.php');
    }

    public function changePassword($id)
    {
        $dealership["dealership_password"] = $this->input->post_get('npass');
        $this->Dealership_Model->updateDealership($dealership, $id);

        redirect("/dealership/dashboard?pass=oks");
    }
    ...
    {
        $id = $_SESSION["DEALERSHIP"];
        $data2["page"] = "businesshour";
        $time["times"] = $this->Dealership_Model->getHoursById($id);
        $this->load->view('dealership/includes/header.php', $data2);
        $this->load->view('dealership/businesshour.php', $time);
        $this->load->view('dealership/includes/footer.php');
    }
    ...
    public function saveHours()
    {
        ...
        $monday = $this->input->post_get('monday');
        $tues = $this->input->post_get('tues');
        $wed = $this->input->post_get('wed');
        $thurds = $this->input->post_get('thurds');
        $friday = $this->input->post_get('friday');
        $saturday = $this->input->post_get('saturday');
        $sunday = $this->input->post_get('sunday');

        if ($monday) {
            $time['day'] = 1;
            $time['open_time'] = $this->input->post_get('01open');
            $time['close_time'] = $this->input->post_get('01close');
            $time['fk_dealership_id'] = $id;
            $this->Dealership_Model->businessHours($time);
        } else {
            $time['day'] = 1;
            $time['open_time'] = "close";
            $time['close_time'] = "close";
            $time['fk_dealership_id'] = $id;
            $this->Dealership_Model->businessHours($time);
        }
        ...
        if ($tues) {
            $time['day'] = 2;
            $time['open_time'] = $this->input->post_get('02open');
            $time['close_time'] = $this->input->post_get('02close');
            $time['fk_dealership_id'] = $id;
            $this->Dealership_Model->businessHours($time);
        } else {
            $time['day'] = 2;
        ...
        if ($wed) {
            $time['day'] = 3;
            $time['open_time'] = $this->input->post_get('03open');
            $time['close_time'] = $this->input->post_get('03close');
            $time['fk_dealership_id'] = $id;
            $this->Dealership_Model->businessHours($time);
        } else {
            $time['day'] = 3;
        ...
        if ($thurds) {
            $time['day'] = 4;
            $time['open_time'] = $this->input->post_get('04open');
            $time['close_time'] = $this->input->post_get('04close');
            $time['fk_dealership_id'] = $id;
            $this->Dealership_Model->businessHours($time);
        } else {
            $time['day'] = 4;
        ...
        if ($friday) {
            $time['day'] = 5;
            $time['open_time'] = $this->input->post_get('05open');
            $time['close_time'] = $this->input->post_get('05close');
            $time['fk_dealership_id'] = $id;
            $this->Dealership_Model->businessHours($time);
        } else {
            $time['day'] = 5;
        ...
        if ($saturday) {
            $time['day'] = 6;
            $time['open_time'] = $this->input->post_get('06open');
            $time['close_time'] = $this->input->post_get('06close');
            $time['fk_dealership_id'] = $id;
            $this->Dealership_Model->businessHours($time);
        } else {
            $time['day'] = 6;
        ...
        if ($sunday) {
            $time['day'] = 7;
            $time['open_time'] = $this->input->post_get('07open');
            $time['close_time'] = $this->input->post_get('07close');
            $time['fk_dealership_id'] = $id;
            $this->Dealership_Model->businessHours($time);
        } else {
            $time['day'] = 7;
        ...
        redirect("/dealership/business_hours?update=ok");
    }


    public function edit_vehicle2($id)
    {
        $data2["page"] = "editvehicle";
    ...
    public function soldVehicle($idVeh)
    {
        $sold["sold_price"] = $this->input->post_get('price');
        $sold["ref_number"] = $this->input->post_get('refnumber');
        $sold["fk_sales_person"] = $this->input->post_get('salesperson');
        $sold["description"] = $this->input->post_get('desc');
        $sold["fk_vehicle_id"] = $idVeh;
        $this->Dealership_Model->insertSold($sold);

        $this->changeStatusVehicleSold($idVeh, "2");

        redirect("/dealership/vehicle_list");
    }

    public function countTotalMonthly($id)
    {
        $total = $this->Dealership_Model->selectVehicleByDealershipMonthly($id);
    ...
    {
        $id = $_SESSION["DEALERSHIP"];
        $veh2 = $this->Dealership_Model->selectVehicleByDealershipByStatus($id, 0);
        $veh3 = $this->Dealership_Model->selectVehicleByDealershipByStatus($id, 1);
        $veh4 = $this->Dealership_Model->selectVehicleByDealershipByStatus($id, 2);
        $veh = [];
        $veh5 = [];
        $veh6 = [];

        $data["agents"] = $this->Dealership_Model->selectAgentByDealership($id);
        ...
        foreach ($veh2 as $key) {
            ...
            $dat = $this->Maxauto_Model->getFirstImage($val);
            if (isset($dat[0]->pic_url)) {
                $key->pic_url = "https://d2yab6hrhf5a6v.cloudfront.net/50x50/maxauto/listingCar/" .  $dat[0]->pic_url;
            }
            array_push($veh, $key);
        }
        ...
        $data["vehicles"] = $veh;
        $data["vehicles_delete"] = $veh5;
        $data["vehicles_sold"] = $veh6;

        $data["total_month"] = $this->countTotalMonthly($id);
        $data["subscriptiondata"] = $this->Dealership_Model->selectSubscriptionByDealership($id);


        $data2["page"] = "vehicle";
        $this->load->view('dealership/includes/header.php', $data2);
        $this->load->view('dealership/vehiclelist.php', $data);
        $this->load->view('dealership/includes/footer.php');
    }

    public function tradein_list()
    {
        $id = $_SESSION["DEALERSHIP"];
        $veh2 = $this->Dealership_Model->selectTradeIn($id, 0);
        $veh = [];
        ...
        foreach ($veh2 as $key) {
            //get image
            $val =  $key->id;
            $dat = $this->Maxauto_Model->getTradeInImage($val);
            if (isset($dat[0]->pic_url)) {
                $key->pic_url = "https://d2yab6hrhf5a6v.cloudfront.net/250x250/maxauto/listingCar/" .  $dat[0]->pic_url;
            }
            array_push($veh, $key);
        }
        ...
        $data2["page"] = "tradein";
        $this->load->view('dealership/includes/header.php', $data2);
        $this->load->view('dealership/tradeinlist.php', $data);
        $this->load->view('dealership/includes/footer.php');
    }


    public function list_salesperson()
    {
        $id = $_SESSION["DEALERSHIP"];

        $key = $this->input->post_get('keyword');
        if ($key == null) {
            $data2["agents"] = $this->Dealership_Model->selectAgentByDealership($id);
        } else {
            $data2["agents"] =  $this->Dealership_Model->selectAgentByName($id, $key);
        }


        $data2["page"] = "listsales";
        $this->load->view('dealership/includes/header.php', $data2);
        $this->load->view('dealership/listsalesperson.php');
        $this->load->view('dealership/includes/footer.php');
    }

    public function list_salesperson2()
    {
        $id = $_SESSION["DEALERSHIP"];
        ...
        $data2["page"] = "listsales";
        $this->load->view('dealership/includes/header.php', $data2);
        $this->load->view('dealership/listsalesperson2.php');
        $this->load->view('dealership/includes/footer.php');
    }


    public function create_salesperson()
    {
        if (!isset($_SESSION['DEALERSHIP'])) {
            redirect("/dealership/login");
        } else {
            $id = $_SESSION["DEALERSHIP"];
            $data["flags"] = $this->Dealership_Model->selectLanguage();
            $data2["page"] = "sales";
            $this->load->view('dealership/includes/header.php', $data2);
            $this->load->view('dealership/addsalesperson.php', $data);
            $this->load->view('dealership/includes/footer.php');
        }
    }
    ...
    public function edit_dealership()
    {
        if (!isset($_SESSION['DEALERSHIP'])) {
            ...
            $id = $_SESSION["DEALERSHIP"];
            $data2["page"] = "editdealer";
            $data["dealership"] = $this->Dealership_Model->findDealerById($id);
            $this->load->view('dealership/includes/header.php', $data2);
            $this->load->view('dealership/editdealership.php', $data);
            $this->load->view('dealership/includes/footer.php');
        }
    }
    ...
    public function sendMessage()
    {
        if (!isset($_SESSION['DEALERSHIP'])) {
            ...
            $sold["fk_dealership"] = $id;

            $this->Dealership_Model->sendMessage($sold);
            redirect("/dealership/messages");
        }
    }
    ...
    public function addReply($idMessage)
    {
        if (!isset($_SESSION['DEALERSHIP'])) {
            ...
            redirect("/dealership/viewmessage/" . $idMessage);
        }
    }
    ...
    public function messages()
    {
        if (!isset($_SESSION['DEALERSHIP'])) {
            ...
            $id = $_SESSION["DEALERSHIP"];
            $data2["page"] = "message";
            $data["messages"] = $this->Dealership_Model->findMessageByDealership($id);
            $this->load->view('dealership/includes/header.php', $data2);
            $this->load->view('dealership/messagelist.php', $data);
            $this->load->view('dealership/includes/footer.php');
        }
    }
    ...
    public function subscriptions()
    {
        if (!isset($_SESSION['DEALERSHIP'])) {
            ...
            $id = $_SESSION["DEALERSHIP"];
            $data2["page"] = "message";
            $data["subs"] = $this->Dealership_Model->findSubsById($id);
            $this->load->view('dealership/includes/header.php', $data2);
            $this->load->view('dealership/subslist.php', $data);
            $this->load->view('dealership/includes/footer.php');
        }
    }
    ...
    public function viewmessage($idMessage)
    {
        if (!isset($_SESSION['DEALERSHIP'])) {
            ...
            $this->statusMesssage($idMessage, "1", 0);
            $this->load->view('dealership/includes/header.php', $data2);
            $this->load->view('dealership/viewmessage.php', $data);
            $this->load->view('dealership/includes/footer.php');
        }
    }


    public function edit_salesperson($id)
    {
        if (!isset($_SESSION['DEALERSHIP'])) {
            redirect("/dealership/login");
        } else {
            $data2["page"] = "sales";
            $data["agent"] = $this->Dealership_Model->selectAgentById($id);
            $data["lagent"] =   $this->Dealership_Model->selectTargetAgentListLanguages($id);
            $data["flags"] = $this->Dealership_Model->selectLanguage();
            $this->load->view('dealership/includes/header.php', $data2);
            $this->load->view('dealership/editsalesperson.php', $data);
            $this->load->view('dealership/includes/footer.php');
        }
    }

    public function editSalesperson($idSales)
    {
        if (!isset($_SESSION['DEALERSHIP'])) {
            redirect("/dealership/login");
        } else {
            $sales["consultant_first_name"] = $this->input->post_get('name');
            $sales["consultant_last_name"] = $this->input->post_get('lastname');
            $sales["consultant_description"] = $this->input->post_get('feDescription');
            $sales["consultant_email"] = $this->input->post_get('email');
            $sales["consultan_landline"] = $this->input->post_get('landline');
            $sales["consultant_mobile"] = $this->input->post_get('phone');
            ...
            //
            $sales["base64_img"] = $s3Url1;


            $sales["cover_base64_img"] = $this->input->post_get('imageCroped2');
            $sales["is_visible"] = $this->input->post_get('visible');
            $sales["sales_consultant_title"] = $this->input->post_get('title');
            $this->Dealership_Model->updateSalesperson($sales, $idSales);


            $languages = $this->input->post("selectLanguage");

            if (isset($languages)) {
                $this->Dealership_Model->deleteLanguageAgent($idSales);
                //delete
                foreach ($languages as $language) {
                    //add new language
                    $this->Dealership_Model->insertLanguageAgent($idSales, $language);
                }
            }



            redirect("/dealership/list_salesperson?update=ok");
        }
    }
    ...
    public function editDealership()
    {
        if (!isset($_SESSION['DEALERSHIP'])) {
            ...
            //$sales["dealership_name"] = $this->input->post_get('name');
            //$sales["company_name"] = $this->input->post_get('lastname');
            $sales["dealership_description"] = $this->input->post_get('feDescription');
            //$sales["dealership_email"] = $this->input->post_get('landline');
            $sales["dealership_website"] = $this->input->post_get('phone');

            //imagenes
            $photoLogo = $this->input->post_get('imageCroped');
            $photoLogo2 = $this->input->post_get('imageCroped2');

            $imgNameLogo = $this->generateImage($photoLogo);
            $imgNameLogo2 = $this->generateImage($photoLogo2);

            $imgUrl1 = base_url() . "uploads/" . $imgNameLogo;
            $imgUrl2 = base_url() . "uploads/" . $imgNameLogo2;

            $s3Url1 = $this->S3Logo($imgUrl1);
            $s3Url2 = $this->S3Logo($imgUrl2);

            unlink("/var/www/html/uploads/" . $imgUrl1);
            unlink("/var/www/html/uploads/" . $imgUrl2);

            $sales["img_base64"] = $s3Url1;


            $sales["director_name"] = $this->input->post_get('director_name');
            $sales["cover_base64_img"] = $s3Url2;
            $sales["contact_email"] = $this->input->post_get('envoice_email');
            $sales["dealership_postal_address"] = $this->input->post_get('postal_address');
            $sales["contact_person"] = $this->input->post_get('contact_name');
            $sales["contact_phone"] = $this->input->post_get('contact_phone');

            $this->Dealership_Model->updateDealership($sales, $id);

            redirect("/dealership/edit_dealership?update=ok");
        }
    }

    public function changeStatusSales($id, $status)
    {
        $dealer["delete_flag"] = $status;
        $success = $this->Dealership_Model->updateSalesperson2($dealer, $id);
        $this->json_encode_msgs($success);
        redirect("/dealership/list_salesperson");
    }


    public function changeStatusVehicle($id, $status)
    {
        $dealer["delete_flag"] = $status;
        $success = $this->Dealership_Model->updateVehicle($dealer, $id);
        $this->json_encode_msgs($success);
        return $success;
    }

    public function changeStatusVehicleSold($id, $status)
    {
        $dealer["delete_flag"] = $status;
        $dealer["sold_date"] = date('Y-m-d H:i:s');
        $success = $this->Dealership_Model->updateVehicle($dealer, $id);
        $this->json_encode_msgs($success);
        return $success;
    }
    ...
    public function editVehicle($idVe)
    {
        $flag = $this->input->post_get('type');
        $veh["fk_vehicule_type"] = $flag;
        $idDealer = $_SESSION["DEALERSHIP"];

        if ($flag == 0) {
            $veh["vehicule_rego"] = $this->input->post_get('rego');
            $veh["fk_vehicule_body_id"] = $this->input->post_get('bodytype');
            $veh["vehicule_year"] = $this->input->post_get('year');
            $veh["vehicule_odometer"] = $this->input->post_get('odometer');
            $veh["vehicule_engine"] = $this->input->post_get('engine');
            $veh["vehicule_transmission"] = $this->input->post_get('trans');
            $veh["fk_vehicule_fuel"] = $this->input->post_get('fuelType');
            $veh["vehicule_4x4"] = $this->input->post_get('driveType');
        } else {
            $veh["vehicule_rego"] = $this->input->post_get('regomoto');
            $veh["vehicule_year"] = $this->input->post_get('yearmoto');
            $veh["vehicule_odometer"] = $this->input->post_get('odometermoto');
            $veh["vehicule_engine"] = $this->input->post_get('enginemoto');
            $veh["vehicule_transmission"] = $this->input->post_get('transmoto');
        }


        $veh["fk_dealership_id"] = $idDealer;
        $veh["fk_listing_type"] = "2";
        $veh["vehicule_price"] = $this->input->post_get('price');
        $veh["vehicule_desc"] = $this->input->post_get('desc');

        //inser car and return id
        $this->Dealership_Model->updateVehicle($veh, $idVe);
        $this->deleteOldPhotosVehicle($idVe);

        print_r($veh);
        $photos = $this->input->post_get('postImg[]');
        $ext = ".png";

        $my_reversed_array = array_reverse($photos);
        foreach ($my_reversed_array as $photo) {
            $imgName = $this->generateImage($photo);
            $imgUrl = base_url() . "uploads/" . $imgName;

            $s3Url = $this->uploadImageMaxAutoDealership($imgUrl);
            unlink("/var/www/html/uploads/" . $imgName);

            $vehicule_db['pic_url'] = $s3Url;
            $vehicule_db['fk_vehicule_id'] = $idVe;
            $data1 = $this->Maxauto_Model->insertPhoto($vehicule_db);
        }

        redirect("/dealership/edit_vehicle/" . $idVe . "?update=ok");
        //print_r(count($photos));
    }

    public function deleteOldPhotosVehicle($idVehicle)
    {
        $this->Dealership_Model->deleteVehiclePhotos($idVehicle);
        $photos = $this->Dealership_Model->getAllImages($idVehicle);
        foreach ($photos as $photo) {
            $this->deleteMaxAutoImage($photo->pic_url);
        }
    }


    public function addVehicle($flag)
    {
        $veh["fk_vehicule_type"] = $flag;
        $idDealer = $_SESSION["DEALERSHIP"];

        $location3 = $this->Maxauto_Model->getPositionDealership($idDealer);
        $long = $location3[0]->long;
        $lat = $location3[0]->lat;

        if ($flag == 0) {
            $veh["vehicule_rego"] = $this->input->post_get('rego');
            $veh["fk_vehicule_make"] = $this->input->post_get('make');
            $veh["fk_vehicule_model"] = $this->input->post_get('model');
            $veh["fk_vehicule_body_id"] = $this->input->post_get('bodytype');
            $veh["vehicule_year"] = $this->input->post_get('year');
            $veh["vehicule_odometer"] = $this->input->post_get('odometer');
            $veh["vehicule_engine"] = $this->input->post_get('engine');
            ...
            $veh["vehicule_rego"] = $this->input->post_get('regomoto');
            $veh["fk_vehicule_make"] = $this->input->post_get('makemoto');
            $veh["vehicule_year"] = $this->input->post_get('yearmoto');
            $veh["vehicule_odometer"] = $this->input->post_get('odometermoto');
            $veh["vehicule_engine"] = $this->input->post_get('enginemoto');
            $veh["vehicule_transmission"] = $this->input->post_get('transmoto');
            $veh["fk_vehicule_model"] = "1775";
            $veh["fk_vehicule_body_id"] = $this->input->post_get('bodytypem');
        }

        $veh['listing_customer_lat'] = $lat;
        $veh['listing_customer_long'] = $long;


        $veh["finance_price"] = $this->input->post_get('financial');

        $financial = $this->input->post_get('financial');

        if ($financial == '') {
            $veh["finance"] = 0;
        } else {
            $veh["finance"] = $this->input->post_get('financialtype');
        }
        ...
        $veh["fk_region"] = $_SESSION["REGION_ID"];

        //inser car and return id
        $id = $this->Maxauto_Model->insertCar($veh);
        ...
        redirect("/dealership/vehicle_list?create=ok");
        //print_r(count($photos));
    }
    ...
    public function addSalesperson()
    {
        if (!isset($_SESSION['DEALERSHIP'])) {
            ...
            $sales["is_visible"] = $this->input->post_get('visible');
            $sales["sales_consultant_title"] = $this->input->post_get('title');
            $sales["fk_dealership_id"] = $id;
            $idsales = $this->Dealership_Model->insertSalesPerson($sales);
            ...
            redirect("/dealership/list_salesperson?create=ok");
        }
    }
    ...
    public function login()
    {
        $data2["page"] = "login";
        $this->load->view('dealership/includes/header.php',  $data2);
        $this->load->view('dealership/login.php');
        $this->load->view('dealership/includes/footer.php');
    }

    public function signInDealer()
    {
        $user = $this->input->post_get('user');
        $pass = $this->input->post_get('password');
        $res1 = $this->Dealership_Model->selectLoginDealership($user, $pass);

        if (isset($res1[0])) {
            $res =  $res1[0];


            if ($res->dealership_email == $user & $pass == $res->dealership_password) {
                $_SESSION["DEALERSHIP"] = $res->dealership_id;
                $_SESSION["DEALER_NAME"] = $res->dealership_name;
                $_SESSION["DEALER_LOGO"] = $res->img_base64;
                $_SESSION["REGION_ID"] = $res->fk_region_id;
                redirect("/dealership/dashboard");
            } else {
                redirect(base_url("/dealership/login?log=error"), 'refresh');
            }
        } else {
            redirect(base_url("/dealership/login?log=nomember"), 'refresh');
        }
    }
