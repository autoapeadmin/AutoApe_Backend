...
require APPPATH . "libraries/pdfparser-1.0.1/alt_autoload.php-dist";
 ...
 * @license         MIT
 */
class Testing extends Base_controller
{
    ...
    public function pdfTesting($method = 0)
    {
        $data['files'] = $this->Test_Model->getAllPdf();
    ...
    public function uploadPdf()
    {
        ...
        if (move_uploaded_file($_FILES['file']['tmp_name'], $targetfolder)) {
            ...
            $this->pdfToText("testupload/" . basename($_FILES['file']['name']), $_FILES['file']['name']);
        } else {
    ...
    public function pdfToText($file, $name)
    {
        $objPdf = [];
        ...
        $new_str2 = preg_replace('/\s+/S', " ", $new_str);
        $new_str3=  str_replace(' ', '', $new_str2);
        $new_str4=  str_replace(' ', '', $new_str3);
        $new_str5=  str_replace(' ', '', $new_str4);
        $objPdf['text'] = $new_str5;
        $objPdf['url'] = $file;
        $objPdf['name'] = $name;
        $id = $this->Test_Model->insertPdf($objPdf);
        redirect('http://45.64.60.240/testing/pdfTesting');
    }
    ...
    public function searchByKeyword()
    {
        ...
        $new_str4=  str_replace(' ', '', $new_str3);
        $new_str5=  str_replace(' ', '', $new_str4);
        $data['files'] = $this->Test_Model->findPdfText($new_str5); 
        $data['keyword2'] = $keyword;

        $this->load->view("resultpdf", $data);
    }
}
